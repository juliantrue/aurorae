...
type Rank = { id: number; key: string; latinName: string; precedence?: number; riteVersionId: number };
type Mov = { id: number; key: string; name: string; offsetFromEaster: number; rankId: number; riteVersionId: number };
type Fix = { id: number; key: string; name: string; month: number; day: number; rankId: number; riteVersionId: number };
type Sea = { id: number; key: string; seasonId: number; weekNumber: number; weekday: string; rankId: number; riteVersionId: number };
...
function seedBasicDB() {
  ...
  const movableFeasts: Mov[] = [
    ...
    { id: 11, key: "EASTER_SUNDAY", name: "Easter Sunday", offsetFromEaster: 0, rankId: 2, riteVersionId: 1 },
    { id: 12, key: "EASTER_SUNDAY", name: "Easter Sunday", offsetFromEaster: 0, rankId: 4, riteVersionId: 2 },
    { id: 13, key: "EASTER_SUNDAY", name: "Easter Sunday", offsetFromEaster: 0, rankId: 6, riteVersionId: 3 },

    // Ash Wednesday (OF & EF) — explicit in 1970 only to show direct match
    { id: 14, key: "ASH_WEDNESDAY", name: "Ash Wednesday", offsetFromEaster: -46, rankId: 6, riteVersionId: 3 },

    // Corpus Christi: present in 1962 only to test inheritance from parent
    { id: 15, key: "CORPUS_CHRISTI", name: "Corpus Christi", offsetFromEaster: 60, rankId: 4, riteVersionId: 2 },
  ];

  // Fixed feasts (Gregorian month/day)
  const fixedFeasts: Fix[] = [
    // Annunciation, present at 1970 level
    { id: 21, key: "SOLEMNITY_ANNUNCIATION", name: "Annuntiatio Domini", month: 3, day: 25, rankId: 6, riteVersionId: 3 },

    // An older fixed feast only at 1570 to test grandparent inheritance
    { id: 22, key: "OLD_FESTUM", name: "Festum Vetus", month: 2, day: 10, rankId: 2, riteVersionId: 1 },
  ];
